...
After `--mode full`, follow `format-pasted-recipe.md`, `home-categories.md`, and `update-recipe-prompt-gallery.md`: derive slug, normalise tags, write `_recipes/<slug>.md`, place `images/<slug>.png`, run `scripts/generate_card_thumbnails.py`, create `prompts/_recipes/<slug>.md`. Stop and ask before overwriting any of these files. Never commit.

`ocr` mode prints text — no scaffolding unless asked. `image` / `prompt` modes produce an image — only overwrite `images/<slug>.png` on explicit user confirmation.
