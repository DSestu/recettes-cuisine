...
### [ ] B1. Encode pilot's source + subfolder

- **Acceptance**
  - `images/pates_sauce_tomate.webp` exists at q90.
  - `images/pates_sauce_tomate/*.webp` for each step photo.
  - PNG originals not yet deleted.
- **Verify**
  - `file images/pates_sauce_tomate.webp` reports WebP.
  - Pillow opens each new file without error.
- **Files**: 1 source + 8 inline images encoded (no code change).
...
- **Acceptance**
  ...
  - Zoom overlay reads `images/full/<slug>.webp` instead of the source.
- **Verify**
  - `bundle exec jekyll serve`; DevTools Network on pilot — hero, inline, zoom all `.webp`. Other recipes still render (still using `.png`).
